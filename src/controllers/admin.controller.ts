...
adminController.processSignup = async (req: AdminRequest, res: Response) => { //Admin request Requestdan etands bolib yasalgan interface
    try {
...
adminController.checkAuthSession = async (req: AdminRequest, res: Response) => { //Test uchun yozilgan ushbu Apida agar body qismida rreq.session.member mavjud bolsa uni Hi burakni yubordik.
    try {
...
export default adminController; //default orqali export qilsak filening umumiy yaxlit natijasini export qilgan bolamiz. Agar export qilinadigan natija bir nechta bolsa defolt sozi ishlatilmaydi.