...
class ProductService {
  ...
  public async rateProduct(member: Member, input: any): Promise<void> {
    ...
    // ⚠️ YANGI: foydalanuvchi haqiqatan ham shu mahsulotni SOTIB OLIB,
    // buyurtmasi FINISH holatiga yetganini tekshiramiz. Avval bu tekshiruv
    // umuman yo'q edi — frontend faqat "Rate" tugmasini Finished Orders
    // ichida ko'rsatib "yashirgan" edi, lekin backend'ning o'zi
    // to'g'ridan-to'g'ri so'rov orqali chaqirilganda hech narsani
    // tekshirmasdi, ya'ni sotib olinmagan mahsulotni ham baholash mumkin
    // edi.
    const finishedOrderIds = await this.orderModel
      .find({ memberId: member._id, orderStatus: OrderStatus.FINISH })
      .distinct("_id");
